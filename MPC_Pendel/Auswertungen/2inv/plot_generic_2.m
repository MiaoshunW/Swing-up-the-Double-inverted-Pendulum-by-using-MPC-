...
dt = 1/framerate;
tv = 0:dt:tmax;
alpha = interp1(t,-alpha(:,2)+pi,tv);   % verändert dies die Werte??? bzw muss das auch für alpha2 gemacht werden
x = interp1(t,x(:,2),tv);
...
try
    ...
    for j = 1:floor(tmax/dt)
        ...
        if alpha2(j) > pi
            alpha2 = alpha2(j) -2*pi;
        end
        
        h2.Matrix = makehgtform('translate',[x(j) 0 0]);
        h3.Matrix = makehgtform('zrotate',abs(alpha(j)-pi));    % überprüfen ob Betrag notwendig da oben bereits alpha umgewandelkt wird
        h4.Matrix = makehgtform('translate',[0 -0.17 0],'zrotate' ,-abs(alpha(j)-pi),'zrotate' ,abs(alpha2(j)-pi));
