...
dt = 1/framerate;
tv = 0:dt:tmax;
alpha = interp1(tout,-x(:,3)+pi,tv);
x = interp1(tout,x(:,1),tv);
% alpha1 = interp1(t,-mpc(:,5)+pi,tv);
% x1 = interp1(t,mpc(:,3),tv);

mpc = importfile6('17b.dat');
x_op = mpc(:,3);
alpha_op = -mpc(:,5)+pi;
t_op = mpc(:,1);
...
%         h4.Matrix = makehgtform('translate',[x1(j) 0 0]);
%         h5.Matrix = makehgtform('zrotate',alpha1(j));
