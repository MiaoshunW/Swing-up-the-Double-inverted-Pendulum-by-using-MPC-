...
enum {
 ...
 /* mode: second order correction refinement. */
  IP_Mode_SOCRefine = 2,
 /* mode: second order correction for rescue of line search. */
  IP_Mode_SOCRescue = 3,
 ...
 /* stage status: optimal solution found. */
  IP_Status_Optimal = 30,
 /* stage status: optimal solution found of barrier problem. */
  IP_Status_BarrierOptimal = 40,
 /* stage status: optimal solution found of penalty problem. */
  IP_Status_PenaltyOptimal = 50,
 /* stage status: search direction is zero. */
  IP_Status_SearchDirectionZero = 60,
 /* stage status: too big. */
  IP_Status_TooBig = 70,
 /* stage status: primal iterates diverge. */
  IP_Status_Diverging = 80,
 /* stage status: NaN evaluation. */
  IP_Status_NaN = 90,
 /* stage status: kkt matrix not positive definite. */
  IP_Status_NotPosDef = 100,
 /* stage status: kkt matrix singular. */
  IP_Status_Singular = 110,
 /*-----------------------------------------------------------------
  *  Worhp Line constants
  *-----------------------------------------------------------------*/
 /* mode: standard. */
  Line_Mode_Std = 1,
 /* mode: second order correction refinement. */
  Line_Mode_SOCRefine = 2,
 /* mode: second order correction rescue of line search. */
  Line_Mode_SOCRescue = 3,
 /* method: filter. */
  Line_Method_Filter = 10,
 /* method: merit function. */
  Line_Method_Merit = 20,
  ...
  *-----------------------------------------------------------------*/
  ...
  Check_KKT = 104,
  Create_QP = 105,
  Hessian_Regularisation = 106,
  Solve_QP = 107,
  Post_QP = 108,
  Find_Stepsize = 109,
  Update_Point = 110,
  Recovery_Strategies = 111,
  Finalise = 112,
  SLP_step = 113,
  Solve_Feasibility_QP = 114,
  Post_Optimum = 115,
  Pre_BStep = 116,
  BStep = 117,
  Post_QP_Sens = 118,
  Worhp_IP_SelectMode = 119,
  Worhp_IP_Estimate = 120,
  Worhp_IP_Prepare = 121,
  Worhp_IP_Check = 122,
  Worhp_IP_UpdateBarrier = 123,
  Worhp_IP_UpdatePenalty = 124,
  Worhp_IP_Factorize = 125,
  Worhp_IP_Regularize = 126,
  Worhp_IP_Step = 127,
  Worhp_Line_Prepare = 128,
  Worhp_Line_Update = 129,
  Worhp_Line_Check = 130,
  Worhp_Line_Correct = 131,
  Worhp_Line_Reduce = 132,
 /*-----------------------------------------------------------------
...
typedef struct WorkspaceStruct {
 ...
 /* WORHP IP: barrier parameter for equality constraints */
  double IP_barrierEq;
 ...
 /* WORHP IP: feasibility of barrier problem */
  double IP_BarrFeas;

 /* WORHP IP: complementarity of barrier problem */
  double IP_BarrCompl;
 ...
 /* WORHP IP: scaling factor for objective in KKT conditions */
  double IP_ScaleKKTObj;

 /* WORHP IP: scaling factor for feasibility in KKT conditions */
  double IP_ScaleKKTFeas;

 /* WORHP IP: scaling factorF of objective */
  double IP_ScaleF;
 ...
 /* WORHP Line: current primal stepsize */
  double Line_AlphaPrimal;

 /* WORHP Line: primal stepsize before second-order-correction */
  double Line_Alpha0Primal;

 /* WORHP Line: maximum primal stepsize */
  double Line_AlphaMaxPrimal;

 /* WORHP Line: current dual1 stepsize */
  double Line_AlphaDual1;

 /* WORHP Line: dual1 stepsize before second-order-correction */
  double Line_Alpha0Dual1;

 /* WORHP Line: maximum dual1 stepsize */
  double Line_AlphaMaxDual1;

 /* WORHP Line: current dual2 stepsize */
  double Line_AlphaDual2;

 /* WORHP Line: dual2 stepsize before second-order-correction */
  double Line_Alpha0Dual2;

 /* WORHP Line: maximum dual2 stepsize */
  double Line_AlphaMaxDual2;
 ...
 /* index map for HM without Xeq */
  ...
  size_t dim_bstepCheckQPLx;
  size_t dim_bstepCheckQPLy;
  size_t dim_bstepCheckQPLz;
  size_t dim_bstepSaveAPert;
  size_t dim_bstepSaveCPert;
 ...
 /* sge: Vector for Lx of QP during BStep feas. ref. */
  double *bstepCheckQPLx;

 /* sge: Vector for Ly of QP during BStep feas. ref. */
  double *bstepCheckQPLy;

 /* sge: Vector for Lz of QP during BStep feas. ref. */
  double *bstepCheckQPLz;

 /* sge: Vector to save perturbation of equality constraints */
  double *bstepSaveAPert;

 /* sge: Vector to save perturbation of inequality constraints */
  double *bstepSaveCPert;
 ...
 /* index vector for SBFGS */
  ...
  size_t dim_IP_MuParam;
  size_t dim_IP_DXDSDMu;
 ...
 /* WORHP IP: search direction of X, S and Mu */
  double *IP_DXDSDMu;
 ...
 /* WORHP IP: residual of X, S and Mu */
  double *IP_ResXSMu;
 ...
 /* WORHP LINE: mode of line search. */
  int Line_Mode;

 /* WORHP LINE: status after evaluation of Line stage. */
  int Line_StageStatus;

 /* WORHP LINE: selects different filter acceptance criteria. */
  int Line_FilterCriteria;
 ...
 /* WORHP IP: flag for evaluation of barrier feasibility */
  bool IP_EvalBarrFeas;
...
typedef struct ParamsStruct {
 ...
 /* WORHP IP: method for init dual variables */
  int IP_DualInitMethod;

 /* WORHP IP: method for init dual box variables */
  int IP_DualBoxInitMethod;

 /* WORHP IP: max iterations for iterative refinement */
  int IP_ItRefMaxIter;

 /* WORHP LINE: line search method (filter, merit function) */
  int Line_Method;

 /* WORHP LINE: strategy for choosing primal stepsize */
  int Line_AlphaPrimal;

 /* WORHP LINE: strategy for choosing first dual stepsize */
  int Line_AlphaDual1;

 /* WORHP LINE: strategy for choosing second dual stepsize */
  int Line_AlphaDual2;
