...
const double dis_times[] = { 0.0, 0.05, 0.2, 0.3, 0.45, 0.55 };
//{ 0.0, 0.06, 0.19, 0.36, 0.53, 0.625 };
		...
		for (int K = 0; K < 80; K++) { //INFINITY
			...
			// echtes System integrieren
				...
				if (thrsh == ph_real.n_dis - 1) {
					mpcfile << base_time_real + sync_time_estim << "s: not enough space in real pendulum time vector!\n";
					exit(6);
				}
				//lineare Interpolation
			...
			// Übergabepunkt für reales System festhalten
				...
				start_real[ph_real.n_ode - 1] = 0.0; //Energie wieder bei 0 starten
			...
			// Textausgabe
				mpcfile << "Unabänderliche Steuersignale: " << endl;
				for (i = 0; i < thrsh; i++)
					mpcfile << base_time_real + ph_simu.T[i] << " s: " << ph_simu.u(i, 0) << endl;
				mpcfile << endl << "Erwartete Zustände: ";
				for (i = 0; i <= thrsh; i++) {
					mpcfile << endl << base_time_optim + ph_simu.T[i] << " s: ";
					mpcfile3 << base_time_optim + ph_simu.T[i] << ", " << ph_simu.u(i, 0);
