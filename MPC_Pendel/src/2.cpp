...
//double start[] = { 0, 0, 0.01, 0, 0 }; //MOD
double start[] = { 0, 0, 3.1416, 0, 0 };
double ziel[] = { 0, 0, 0, 0, 0 };

double weight[] = { 0, 0.2, 500, 100, 0.1 };

double up_bnd[] = {0.5, 0.38, 0.5, 1e20, 25, 100};
double lw_bnd[] = {-0.5, -0.38, -0.5, -1e20, -25, -1000};

int error;

double weightmax;
double gl,l,J_eff,E2max;
...
public:
	...
	double obj() {
		...
		double dalpha = fmod(x(n_dis - 1, 2) - ziel[2] + 3.1415926535898, 6.2831853071796) - 3.1415926535898;

		double tmp = (x(n_dis - 1, 0) - ziel[0]);
		double ret = tmp*tmp * weight[0];
		tmp = (x(n_dis - 1, 1) - ziel[1]);
		ret += tmp*tmp * weight[1];
		ret += dalpha*dalpha * weight[2];
		tmp = (x(n_dis - 1, 3) - ziel[3]);
		ret += tmp*tmp * weight[3];
		ret += x(n_dis - 1, 4) * weight[4];
		ret *= weightmax;

		double ret2 = 0;
		ret2 = cos(x(n_dis - 1, 2)) - cos(ziel[2]);
		ret2 *= x(n_dis - 1, 1)*x(n_dis - 1, 3)*l + gl;
		ret2 += x(n_dis - 1, 3)*x(n_dis - 1, 3)*J_eff;
		ret2 += x(n_dis - 1, 1)*x(n_dis - 1, 1);
		ret2 *= ret2 * E2max;
...
public:
	...
	MPC_Pendel_real(int dis) : TransWorhp("MPC_Pendel_real", dis, 4, 1, 0, 0, 0) {}
...
int main(int argv, char* argc[]) {
	...
	for (i = 0; i < 5; i++) {
		...
		weightmax += max_bnd[i] * max_bnd[i] * weight[i];
	}
	weightmax = 1 / weightmax;

	l = 0.5;
	gl = 9.81*l;
	J_eff = (4.0*l*l + 0.02*0.02)/12.0;
	E2max = max_bnd[1] * max_bnd[1] + J_eff*max_bnd[3] * max_bnd[3];
