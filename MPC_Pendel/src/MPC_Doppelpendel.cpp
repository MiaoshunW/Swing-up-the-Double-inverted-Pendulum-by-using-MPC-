...
//Pendulum 1 swinged up ,Pendulum 2 should hang down  <Reference>
...
const double pre_calc_time = 0.0; // Pre-calculated time
...
inline double interp1(const double x[2], double y[2], double xi) {                 
...
const double  J1 = ((m1 * l1 * l1) / 12);	    //Moment of inertia pendulum 1 [kg m^2]  	
const double  J2 = ((m2 * l2 * l2) / 12);	    //Moment of inertia pendulum 2 [kg m^2]
...
public:
	...
	double obj() {                                                                   
		...
		//    = T1 + T2 + T3 = 0 + m1*g*a1*cos¦È1 + m2g(l1*cos¦È1 + a2*cos¦È2)
	...
	bool obj_diff(DiffStructure &s) {
		...
		s(0, x_indexode(2)) += -2 *(cos(x(j, 2)) - cos(ziel[2]))*(l1*g*m2 + g*m1*a1)*sin(x(j, 2))*weight[7]; 
...
void initialize_weights() {			
	...
	double E2max = m1*g*a1 + m2*g*l1 + m2*g*a2;  // the energy of pendulum
...
int main(int argv, char* argc[]) {
	...
	try
	{
		...
		//lin Interp. others, MATLAB course, then save, and import, also for control
		for (i = 0; i < n_dis[0]; i++) {		
			control[i] = 0.0;                
		...
		for (int K = 0; K < 80; K++) { //INFINITY
			...
			n_dis[0] = temp_n_dis;
			p_ph_optim[0] = p_ph_optim[n_dis[0] - 1]; //only serves slim code
	...
	if (sync_time_current < sync_time_estim - pre_calc_time)
			...
			if (fabs(dis_times[thrsh] - sync_time_current) > 0.0001)
			{
				...
				start_real[p_ph_real[0]->n_ode-1] = 0.0; 
