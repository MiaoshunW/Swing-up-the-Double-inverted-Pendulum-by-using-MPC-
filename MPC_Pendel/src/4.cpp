...
double weight[] = { 0.01, 0.1, 1, 0.1, 0.01, 1 };
// Wagenlage, Wagengeschw., Pendellage, Pendelgeschw., Stellenergie, Pendelenergie^2
// werden jeweils auf Grenze, bzw. Pi, normiert 
const double up_bnd[] = { 0.5, 0.38, 0.5, 1e20, 25, 100 };
...
public:
	...
	double obj() {
		...
		double dalpha = fmod(x(n_dis - 1, 2) - ziel[2] + Pi, Pi2) - Pi;

		double ret = 0;
		ret = cos(x(n_dis - 1, 2)) - cos(ziel[2]);
		ret *= x(n_dis - 1, 1)*x(n_dis - 1, 3)*l + gl;
		ret += x(n_dis - 1, 3)*x(n_dis - 1, 3)*J_eff;
		ret += x(n_dis - 1, 1)*x(n_dis - 1, 1);
		ret *= ret * weight[5];
