...
double start_real[] = { 0, 0, 0, Pi, 0, 0 };
...
const double ext_estim_time = 0.05;
...
public:
	...
	//*
	int step() override {
		if (0.001*(double)GetTickCount()-start_time_optim < max_time)
			return true;
		else {
			printf("time exceeded, returning false\n");
			time_exceeded = true;
			return false;
		}
	}
...
public:
	...
	void GetXTitle(int d, char *s) {
		...
		if (d == 5) strcpy(s, "cumulated objective function");
	}
	...
	void ode(double *dx, double t, const double *x, const double *u, const double *p) {
		...
		alpha = fabs(fmod(x[3] - ziel[2] + Pi, Pi2) - Pi);

		tmp = (x[1] - ziel[1]);
		ret = tmp*tmp * weight[1];

		ret += weight[2] * Pi * alpha;

		tmp = (x[4] - ziel[3]);
		ret += tmp*tmp * weight[3];

		ret += u[0] * u[0] * weight[4];

		tmp = cos(x[3]) - cos(ziel[2]);
		tmp *= x[1] * x[4] * l + gl;
		tmp += x[4] * x[4] * J_eff;
		tmp += x[1] * x[1];
		ret += tmp*tmp* weight[5];
...
double subroutine(double *dis_times, int* errcode, int* balanced, int c) {
	...
	double calc_time_estim_next, compare_time, temp_time[3], temp_u[6];

	char filen[20] = "mpc2_";
	sprintf(filen, "mpc2_%d.dat", c);
	ofstream mpcfile2(filen);
	...
	try
	{
		...
		for (state = 0; state < ph_optim.n_ode; state++)
			states[i][state] = (start[state] * (dis_times[ph_optim.n_dis - 1] - dis_times[i]) + ziel[state] * dis_times[i]) / dis_times[ph_optim.n_dis - 1];
		}

		start_time_optim = 0.001*(double)GetTickCount();
		time_exceeded = false;
		folder_optim.Init();
		...
		*errcode = 0;
		...
		for (int K = 0; K < 60; K++) {
			...
			// Startschätzung aus Anfangszustand und Steuerung integrieren
				...
				if (sync_time_estim >= dis_times[5]) {
					*errcode = 5;
					mpcfile2 << "sync_time_estim >= dis_times[5]";
					mpcfile2.close();
					return 0;
				}
			...
			// Steuerung für echtes System festhalten
			for (i = 0; i < ph_optim.n_dis; i++)
				temp_u[i] = ph_optim.u(i, 0);
			...
			// Optimierung
				...
				for (i = 1; compare_time < dis_times[ph_optim.n_dis-1]; i++) {
					if (compare_time >= dis_times[5]) {
						*errcode = 5;
						mpcfile2 << "compare_time >= dis_times[5]";
						mpcfile2.close();
						return 0;
					}
			...
			//echtes System reinitialisieren und Steuerfolge eintragen
			for (state = 0; state < ph_real.n_ode; state++)
				ph_real.X[ph_real.x_index(0, state)] = start_real[state];
			for (i = 0; i < ph_optim.n_dis; i++)
				ph_real.X[ph_real.u_index(i, 0)] = temp_u[i];
			...
			if (sync_time_estim > ph_optim.T[ph_optim.n_dis - 1]) {
				*errcode = 5;
				mpcfile2.close();
				return 0;
			}
			...
			// mehr als die Hälfte kürzer als gedacht ?
			...
			// echtes System integrieren
				...
				// Steuerung festlegen
				thrsh2 = 1;
				if (sync_time_current >= dis_times[5]) {
					*errcode = 5;
					mpcfile2 << "sync_time_current >= dis_times[5]";
					mpcfile2.close();
					return 0;
				}
				...
				//Zwischenspeichern
				for (i = 0; i < ph_real.n_dis; i++)
					temp_u[i] = ph_real.u(i, 0);
				//lineare Interpolation
				ph_real.X[ph_real.u_index(thrsh + 1, 0)] = (ph_real.u(thrsh, 0)*(sync_time_estim - ph_real.T[thrsh - 1])
					+ ph_real.u(thrsh - 1, 0)*(ph_real.T[thrsh] - sync_time_estim)) / (ph_real.T[thrsh] - ph_real.T[thrsh - 1]);
				ph_real.T[thrsh + 1] = sync_time_estim;
				for (i = thrsh; i > thrsh2; i--) {
					ph_real.X[ph_real.u_index(i, 0)] = ph_real.u(i - 1, 0);
					ph_real.T[i] = ph_real.T[i - 1];
				}
				ph_real.X[ph_real.u_index(thrsh2, 0)] = (ph_real.u(thrsh2, 0)*(sync_time_current - ph_real.T[thrsh2 - 1])
					+ ph_real.u(thrsh2 - 1, 0)*(ph_real.T[thrsh2] - sync_time_current)) / (ph_real.T[thrsh2] - ph_real.T[thrsh2 - 1]);
				ph_real.T[thrsh2] = sync_time_current;
				thrsh++;
			...
			// System integrieren
				...
				//*/
				for (i = 0; i <= thrsh; i++)
					if ((ph_real.x(i, 0)>up_bnd[1]) || (ph_real.x(i, 0)<lw_bnd[1])) {
						*errcode = 3;
					}

			// Änderungen rückgängig machen
				for (i = thrsh2; i <= thrsh; i++) {
					ph_real.X[ph_real.u_index(i, 0)] = temp_u[i];
					ph_real.T[i] = dis_times[i];
				}
				ph_real.n_dis = 6;

			// Anfangszustand Startschätzung festlegen
				ph_simu.X[ph_simu.x_index(0, 0)] = ph_real.x(thrsh2, 0);
				ph_simu.X[ph_simu.x_index(0, 1)] = ph_real.x(thrsh2, 1);
				ph_simu.X[ph_simu.x_index(0, 2)] = ph_real.x(thrsh2, 3);
				ph_simu.X[ph_simu.x_index(0, 3)] = ph_real.x(thrsh2, 4);
				ph_simu.X[ph_simu.x_index(0, 4)] = 0.0; //Energie wieder bei 0 starten

			// Übergabepunkt für reales System festhalten
				for (state = 0; state < ph_real.n_ode; state++) {
					start_real[state] = ph_real.x(thrsh, state);
				}
			...
			start_time = 0.001*(double)GetTickCount();
				...
				ph_simu.X[ph_simu.u_index(thrsh, 0)] = (ph_real.u(thrsh2, 0)*(sync_time_estim - ph_real.T[thrsh2 - 1])
			...
			// Übergabepunkt für reales System festhalten
				...
				start_real[0] = start[0];
				start[1] = ph_real.x(thrsh, 1);
				start_real[1] = start[1];
				start_real[2] = ph_real.x(thrsh, 2);
				start[2] = ph_real.x(thrsh, 3);
				start_real[3] = start[2];
				start[3] = ph_real.x(thrsh, 4);
				start_real[4] = start[3];
				start[4] = 0.0; //Energie wieder bei 0 starten
				start_real[5] = ph_real.x(thrsh, 5);
		...
		if ((fabs(start[2]) < 0.1) && (fabs(start[3]) < 1.0))
			*balanced = 1;
		else
			*balanced = 0;
		mpcfile2.close();
