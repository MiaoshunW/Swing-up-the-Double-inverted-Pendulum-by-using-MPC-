...
/*
		...
		tmp = fmod(x(n_dis - 1, 2) - ziel[2] + 3.1415926535898, 6.2831853071796) - 3.1415926535898;
		ret += tmp*tmp * weight[2];
		tmp = (x(n_dis - 1, 3) - ziel[3]);
		ret += tmp*tmp * weight[3];
		ret += x(n_dis - 1, 4) * weight[4];
		
		ret = (ret+scale[0])*scale[1];
//*/
...
//*
		...
		double gl3 = 14.715;
		double l2b2 = 0.2504;
		double E2max = 0.00065326787;
		double ret = 0;
		ret = x(n_dis - 1, 3);
		ret *= ret*l2b2/4.0;
		ret += gl3*(cos(x(n_dis - 1, 2)) - 1);
		ret *= ret * E2max;
//*/
...
int main(int argv, char* argc[]) {
	...
	scale[0] = 1000.0*weight[4];
	scale[1] = 1 / (0.16*weight[0] + 0.25*weight[1] + 9.8696044010894*weight[2] + 625.0 * weight[3] + 1100.0 * weight[4]);
