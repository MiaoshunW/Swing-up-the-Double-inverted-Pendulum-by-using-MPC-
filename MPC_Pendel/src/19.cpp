...
public:
	...
	double obj() {
		...
		double tmp, alpha, ret = 0;
		for (j = n_dis - 3; j < n_dis; j++) {
			alpha = fmod(x(j, 2) - ziel[2] + Pi, Pi2) - Pi;

			tmp = (x(j, 0) - ziel[0]);
			ret += tmp*tmp * weight[0] * fabs(alpha) / Pi;

			tmp = (x(j, 1) - ziel[1]);
			ret += tmp*tmp * weight[1] * fabs(alpha) / Pi;

			ret += alpha*alpha * weight[2];

			tmp = (x(j, 3) - ziel[3]);
			ret += tmp*tmp * weight[3] * fabs(alpha) / Pi;

			ret += x(j, 4) * weight[4] * fabs(alpha) / Pi;

			tmp = cos(x(j, 2)) - cos(ziel[2]);
			tmp *= x(j, 1)*x(j, 3)*l + gl;
			tmp += x(j, 3)*x(j, 3)*J_eff;
			tmp += x(j, 1)*x(j, 1);
			ret += tmp*tmp* weight[5] * fabs(alpha)/Pi;
		}
		return ret;
	}
	...
	try
	{
		...
		for (int K = 0; K < 60; K++) { //INFINITY
			...
			// und Anfangszustand Startschätzung festlegen
				...
				ph_real.X[ph_real.x_index(0, 4)] = 0.0; //Energie wieder bei 0 starten
				ph_simu.X[ph_simu.x_index(0, 4)] = 0.0;
