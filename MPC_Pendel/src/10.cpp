		...
		*/
		...
		for (int K = 0; K < 60; K++) { //INFINITY
			...
			for (i = 0; i < ph_real.n_dis; i++) {
				...
				ph_real.X[ph_real.u_index(i, 0)] = ph_optim.u(thrsh + 1, 0)*ph_real.T[i] - ph_optim.u(thrsh, 0)*ph_real.T[i] + tmp2;
				ph_real.X[ph_real.u_index(i, 0)] *= tmp;
			}
